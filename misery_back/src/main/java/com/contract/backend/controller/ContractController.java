...
public class ContractController {
    ...
    public ResponseEntity<ApiResponse<ContractDetailDTO>> getContractDetails(
            @PathVariable("contractId") Long contractId, // <-- ("contractId") 추가!
    ...
    public ResponseEntity<ApiResponse<Void>> addFavorite(
        ...
        try {
            contractService.addFavoriteContract(contractId, uuid);
            return ResponseEntity.ok(ApiResponse.success(null)); // 성공 응답 (데이터 없이)
        } catch (CustomException e) {
            // GlobalExceptionHandler에서 처리되도록 CustomException을 그대로 던짐
            throw e;
        } catch (Exception e) {
            // 기타 예외는 RuntimeException으로 감싸서 GlobalExceptionHandler에서 500 에러로 처리
    ...
    public ResponseEntity<ApiResponse<Void>> removeFavorite(
        ...
        try {
            contractService.removeFavoriteContract(contractId, uuid);
    ...
    /**
     * 사용자의 즐겨찾기 계약 목록 조회 API
     */
    @GetMapping("/favorites")
    public ResponseEntity<ApiResponse<Page<ContractListDTO>>> getMyFavoriteContracts(
            @AuthenticationPrincipal String uuid,
            @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable
    ) {
        try {
            Page<ContractListDTO> favoriteContracts = contractService.getFavoriteContracts(uuid, pageable);
            return ResponseEntity.ok(ApiResponse.success(favoriteContracts));
        } catch (CustomException e) {
            throw e;
        } catch (Exception e) {
            throw new RuntimeException("Failed to retrieve favorite contracts: " + e.getMessage(), e);
        }
    }
}
...
import com.contract.backend.common.repository.FavoriteContractRepository; // 추가
...
public class ContractService {
    ...
    /**
     * 사용자가 접근 가능한 모든 계약 목록을 조회하며, 즐겨찾기 여부를 포함합니다.
     * (기존 getContractsForUser 수정)
     *
     * @param userUuid 사용자 UUID
     * @param pageable 페이지 정보
     * @return 사용자의 계약 목록 (ContractListDTO 형태, 즐겨찾기 여부 포함)
     */
    @Transactional(readOnly = true)
    public Page<ContractListDTO> getContractsForUser(String userUuid, Pageable pageable) {
        ...
        // 사용자가 생성했거나 참여한 계약 페이지 조회
        Page<ContractEntity> contractPage = contractRepository.findContractsByCreatorOrParticipant(user, pageable);

        // 조회된 계약 목록 ID 추출
        List<Long> contractIds = contractPage.getContent().stream()
                                            .map(ContractEntity::getId)
                                            .collect(Collectors.toList());

        // 해당 계약들 중 사용자가 즐겨찾기한 계약 ID 목록 조회
        Set<Long> favoriteContractIds = favoriteContractRepository.findAllByUserAndContractIdIn(user, contractIds) // Repository에 메서드 추가 필요
                                        .stream()
                                        .map(fav -> fav.getContract().getId())
                                        .collect(Collectors.toSet());

        // ContractListDTO로 변환하면서 즐겨찾기 여부 설정
        return contractPage.map(contract -> {
            boolean isFavorite = favoriteContractIds.contains(contract.getId());
            return mapToContractListDTO(contract, isFavorite); // isFavorite 상태 포함
        });
    }
...
public ContractDetailDTO getContractDetails(Long contractId, String requesterUuid) {
     ...
     * 계약을 즐겨찾기에 추가합니다.
     *
     * @param contractId 즐겨찾기할 계약 ID
     * @param userUuid 요청 사용자 UUID
     * @throws CustomException 계약이나 사용자를 찾을 수 없거나, 이미 추가된 경우, 권한이 없는 경우
     */
    @Transactional
    public void addFavoriteContract(Long contractId, String userUuid) {
        UserEntity user = userRepository.findByUuid(userUuid)
                .orElseThrow(() -> new CustomException(CustomExceptionEnum.USER_NOT_FOUND));
        ContractEntity contract = contractRepository.findById(contractId)
                .orElseThrow(() -> new CustomException(CustomExceptionEnum.CONTRACT_NOT_FOUND));

        // 계약 접근 권한 확인 (예: 생성자 또는 참여자인 경우만 즐겨찾기 가능)
        checkContractAccessPermission(contract, user);

        // 이미 즐겨찾기 되어 있는지 확인
        if (favoriteContractRepository.findByUserAndContract(user, contract).isPresent()) {
            // 이미 추가된 경우, 예외를 던지거나 조용히 무시할 수 있습니다. 여기서는 무시.
            logger.info("Contract {} is already favorited by user {}", contractId, userUuid);
            return;
             // throw new CustomException(CustomExceptionEnum.FAVORITE_ALREADY_EXISTS); // 필요시 예외 정의 후 사용
        }

        FavoriteContractEntity favorite = new FavoriteContractEntity(user, contract);
        favoriteContractRepository.save(favorite);
        logger.info("Contract {} added to favorites for user {}", contractId, userUuid);
    }

    /**
     * 계약을 즐겨찾기에서 제거합니다.
     *
     * @param contractId 제거할 계약 ID
     * @param userUuid 요청 사용자 UUID
     * @throws CustomException 계약이나 사용자를 찾을 수 없거나, 즐겨찾기에 없는 경우
     */
    @Transactional
    public void removeFavoriteContract(Long contractId, String userUuid) {
        UserEntity user = userRepository.findByUuid(userUuid)
                .orElseThrow(() -> new CustomException(CustomExceptionEnum.USER_NOT_FOUND));
        ContractEntity contract = contractRepository.findById(contractId)
                .orElseThrow(() -> new CustomException(CustomExceptionEnum.CONTRACT_NOT_FOUND));
        ...
        // 즐겨찾기 존재 여부 확인 및 삭제
        FavoriteContractEntity favorite = favoriteContractRepository.findByUserAndContract(user, contract)
                .orElseThrow(() -> new CustomException(CustomExceptionEnum.FAVORITE_NOT_FOUND)); // FAVORITE_NOT_FOUND 예외 정의 필요

        favoriteContractRepository.delete(favorite);
        // 또는 favoriteContractRepository.deleteByUserAndContract(user, contract);
        logger.info("Contract {} removed from favorites for user {}", contractId, userUuid);
    }

    /**
     * 사용자의 즐겨찾기 계약 목록을 조회합니다.
     *
     * @param userUuid 사용자 UUID
     * @param pageable 페이지 정보
     * @return 즐겨찾기된 계약 목록 (ContractListDTO 형태)
     */
    @Transactional(readOnly = true)
    public Page<ContractListDTO> getFavoriteContracts(String userUuid, Pageable pageable) {
        UserEntity user = userRepository.findByUuid(userUuid)
                .orElseThrow(() -> new CustomException(CustomExceptionEnum.USER_NOT_FOUND));

        Page<ContractEntity> favoriteContractsPage = favoriteContractRepository.findFavoriteContractsByUser(user, pageable);

        // 즐겨찾기 목록은 항상 isFavorite = true 로 설정
        return favoriteContractsPage.map(contract -> mapToContractListDTO(contract, true)); // isFavorite 플래그 추가
    }

    // ContractEntity를 ContractListDTO로 변환하는 헬퍼 메서드 (isFavorite 파라미터 추가)
    private ContractListDTO mapToContractListDTO(ContractEntity contract, boolean isFavorite) {
        Integer currentVersionNumber = null;
        if (contract.getCurrentVersion() != null) {
            currentVersionNumber = contract.getCurrentVersion().getVersionNumber();
        }
        // ContractListDTO에 isFavorite 필드 추가 필요
        ContractListDTO dto = new ContractListDTO(
         ...
         dto.setFavorite(isFavorite); // DTO에 setter 추가 가정
        return dto;
    }

    // 계약 접근 권한 확인 로직 (Helper Method)
    private void checkContractAccessPermission(ContractEntity contract, UserEntity user) {
        ...
        if (!isCreator && !isParticipant) {
             logger.warn("User {} does not have permission to access contract {}", user.getUuid(), contract.getId());
             throw new CustomException(CustomExceptionEnum.UNAUTHORIZED); // 접근 권한 없음
        }
    }
